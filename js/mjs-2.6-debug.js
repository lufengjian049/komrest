...
opobj={
	...
	datesobj:{
		"c60":"丢单时间","c0":"报价终审时间","c1":"Commit申请时间","o1":"预计Sell Out时间","o2":"实际Sell Out时间"
	},
	curdateobj:null,
	res:null,
	totalPrice:null,
...
$(function(){
	...
	$(document).on("tap","#extenddivmenu",function(){
		...
		if(slidemenu.hasClass("show2")){
			...
			$("div.current").css({"-webkit-transform":"translate(0,0) translateZ(0)","-webkit-transition":"-webkit-transform 400ms"});
			$(this).removeData("swipe");
			setTimeout(function(){
				slidemenu.removeClass("hide2").hide();
				$("div.current").css({"-webkit-transform":"dsds","-webkit-transition":"dsds"});
				$("#detailmask").hide();
			},400);
		}else{
			slidemenu.removeClass("hide2").addClass("show2").show();
			$("div.current").css({"-webkit-transform":"translate(-80%,0) translateZ(0)","-webkit-transition":"-webkit-transform 400ms"});
			$("#detailmask").show();
	...
	$(document).on("tap", "#selecttime", function() {
			...
			opobj.curdateobj=_thisb.find(".rowcontent");
...
function backfn(total){
	...
	$("#ultalble").on("longTap","li",function(){
  		var sheight=Util.os.height,curstatus=$("#typehidden").val(),hastwo=[],
  		itemparams={
  	...
  	$(".isearch>span").on("tap",function(){
  		...
  		_this.next().show().next().show().next().css({"right":"30%"});
  	})
  	//时间查询按钮事件
...
function pickdatecallback(backobj){
	if(opobj.curdateobj.hasClass("rowcontent")){
		opobj.curdateobj.html(formatDate(new Date(backobj.date)));
		opobj.curdateobj.next().val(formatDate(new Date(backobj.date)));
	}
	else
...
;(function($, window){
  $.fn.inputinit=function(){
    this.each(function(){
      var _this=$(this),parentnode=_this.parent(),dtype=_this.attr("dtype");
